...
# ============================================================================ #
"""Run build-tree mklq-cpu gate fast-path counter tests."""
...
COUNTER_SOURCE = (
    "MKL-Q mklq-cpu simulator test-accessor gate fast-path counters")

COUNTER_TEST_SUFFIXES = (
    "XFastPathAppliesUncontrolledSingleQubitGate",
    "CnotFastPathAppliesControlledXGate",
    "BuiltInSingleQubitFastPathsMatchMatrices",
    "ControlledBuiltInSingleQubitFastPathsMatchMatrices",
    "SingleControlBuiltInSingleQubitGatesUseDedicatedFastPath",
    "SingleControlBuiltInHadamardGateUsesDedicatedFastPath",
    "SingleControlBuiltInYGateUsesDedicatedFastPath",
    "SingleControlBuiltInRxGateUsesDedicatedFastPath",
    "SingleControlBuiltInRyGateUsesDedicatedFastPath",
    "SingleControlRzUsesDedicatedPhaseFastPath",
...
def select_counter_tests(ctest_listing: str) -> list[str]:
    ...
    for line in ctest_listing.splitlines():
        ...
        if suffix in suffixes:
            found.add(test_name)
    return [test for test in expected_counter_tests() if test in found]


def missing_counter_tests(selected: list[str]) -> list[str]:
    selected_set = set(selected)
    return [
        test for test in expected_counter_tests() if test not in selected_set
    ]
...
def build_report(repo_root: Path, build_dir: Path) -> dict[str, Any]:
    ...
    listing_command = [
        ...
        "-N",
        "-R",
        "mklq_cpu_MKLQCpuTester",
    ]
    listing = command_output(root, listing_command)
    public_build_dir = command_path(root, build_dir)
    ...
    status = ("passed"
              if selected and not missing and failed_count == 0 else "failed")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=(
        "Run MKL-Q CPU gate fast-path counter ctests and write bounded "
        "JSON evidence."))
