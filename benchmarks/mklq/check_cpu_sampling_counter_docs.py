...
"""Check that public CPU sampling counter docs match tracked reports."""
...
DEFAULT_DOC = Path("docs/mklq/cpu-sampling-counters.md")
...
def check_docs(report_inputs: list[Path] | None = None,
    ...
    if current is None:
        return _failed("CPU sampling counter docs are missing", details)
    if summary_status != "passed":
        return _failed("CPU sampling counter summary is not passing", details)
    if REQUIRED_AGGREGATE_NOTE not in generated:
        details["required_note"] = REQUIRED_AGGREGATE_NOTE
        return _failed(
            "CPU sampling counter docs generator is missing the aggregate-count note",
            details)
    if current != generated:
        diff = difflib.unified_diff(
            ...
            tofile="regenerated-cpu-sampling-counters.md",
            lineterm="")
        details["diff"] = "\n".join(list(diff)[:160])
        return _failed(
            "tracked CPU sampling counter docs output differs from regenerated output",
            details)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Check docs/mklq/cpu-sampling-counters.md against tracked bounded "
            "CPU sampling counter reports."))
    parser.add_argument("--reports",
                        ...
                        help=(
                            ...
                            "Directories are searched for *.cpu-counter.json."))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print("CPU sampling counter docs are current: "
