...
METAL_FULL_REGISTER_SAMPLING_SCOPE = (
    ...
    "Metal device-generated draw plus sample-count accumulation")
...
def dense_two_qubit_matrix() -> list[list[float]]:
  return [
      [0.5, 0.5, 0.5, 0.5],
      [0.5, -0.5, 0.5, -0.5],
      [0.5, 0.5, -0.5, -0.5],
...
def build_dense_two_qubit_state_kernel(
    cudaq: Any, qubits: int, layers: int) -> tuple[Any, int, int, int]:
  if qubits < 2:
    raise ValueError("dense two-qubit benchmarks require at least 2 qubits")
  if not hasattr(cudaq, "register_operation"):
    raise RuntimeError("cudaq.register_operation is required")

  cudaq.register_operation(DENSE_TWO_QUBIT_OPERATION_NAME,
                           dense_two_qubit_matrix())

  kernel = cudaq.make_kernel()
  q = kernel.qalloc(qubits)
  state_prep_gate_count = 0
  dense_two_qubit_gate_count = 0
  ...
  for _ in range(layers):
    for index in range(0, qubits - 1, 2):
      kernel.__getattr__(DENSE_TWO_QUBIT_OPERATION_NAME)(q[index],
                                                         q[index + 1])
      dense_two_qubit_gate_count += 1
    for index in range(1, qubits - 1, 2):
      kernel.__getattr__(DENSE_TWO_QUBIT_OPERATION_NAME)(q[index],
                                                         q[index + 1])
      dense_two_qubit_gate_count += 1

  gate_count = state_prep_gate_count + dense_two_qubit_gate_count
  return (kernel, gate_count, state_prep_gate_count,
          dense_two_qubit_gate_count)


def build_controlled_dense_two_qubit_state_kernel(
    cudaq: Any, qubits: int, layers: int) -> tuple[Any, int, int, int]:
  if qubits < 3:
    raise ValueError(
        "controlled dense two-qubit benchmarks require at least 3 qubits")
  if not hasattr(cudaq, "register_operation"):
    raise RuntimeError("cudaq.register_operation is required")

  cudaq.register_operation(CONTROLLED_DENSE_TWO_QUBIT_OPERATION_NAME,
                           dense_two_qubit_matrix())

  dense_kernel, first, second = cudaq.make_kernel(cudaq.qubit, cudaq.qubit)
  dense_kernel.__getattr__(CONTROLLED_DENSE_TWO_QUBIT_OPERATION_NAME)(first,
                                                                      second)

  kernel = cudaq.make_kernel()
  q = kernel.qalloc(qubits)
  state_prep_gate_count = 0
  controlled_dense_two_qubit_gate_count = 0
  ...
  for _ in range(layers):
    for index in range(qubits - 2):
      kernel.control(dense_kernel, q[index], q[index + 1], q[index + 2])
      controlled_dense_two_qubit_gate_count += 1

  gate_count = (
      state_prep_gate_count + controlled_dense_two_qubit_gate_count)
  return (kernel, gate_count, state_prep_gate_count,
          controlled_dense_two_qubit_gate_count)


def build_controlled_swap_state_kernel(
    cudaq: Any, qubits: int, layers: int) -> tuple[Any, int, int, int]:
  if qubits < 3:
    raise ValueError("controlled SWAP benchmarks require at least 3 qubits")

  kernel = cudaq.make_kernel()
  q = kernel.qalloc(qubits)
  state_prep_gate_count = 0
  controlled_swap_gate_count = 0
  ...
  for _ in range(layers):
    for index in range(qubits - 2):
      kernel.cswap(q[index], q[index + 1], q[index + 2])
      controlled_swap_gate_count += 1

  gate_count = state_prep_gate_count + controlled_swap_gate_count
  return (kernel, gate_count, state_prep_gate_count, controlled_swap_gate_count)
...
def run_case(cudaq: Any, target: str, case: str, qubits: int, shots: int,
  ...
  try:
    ...
    if case == "gate-state":
    ...
    elif case == "dense-two-qubit-state":
      kernel, gate_count, state_prep_gate_count, dense_two_qubit_gate_count = (
          build_dense_two_qubit_state_kernel(cudaq, qubits, layers))
      action = lambda: cudaq.get_state(kernel)
      for _ in range(warmups):
        action()
      timings = timed_repeats(action, repeats)
      ...
      metrics.update({
          "gate_count": gate_count,
          "state_prep_gate_count": state_prep_gate_count,
          "dense_two_qubit_gate_count": dense_two_qubit_gate_count,
          "dense_two_qubit_operation_name": DENSE_TWO_QUBIT_OPERATION_NAME,
          "layers": layers,
          "dense_two_qubit_gate_state_throughput_per_second":
              dense_two_qubit_gate_count / median if median > 0 else None,
      })
    elif case == "controlled-dense-two-qubit-state":
      kernel, gate_count, state_prep_gate_count, gate_count_controlled = (
          build_controlled_dense_two_qubit_state_kernel(cudaq, qubits, layers))
      action = lambda: cudaq.get_state(kernel)
      for _ in range(warmups):
        action()
      timings = timed_repeats(action, repeats)
      ...
      metrics.update({
          "gate_count":
              gate_count,
          "state_prep_gate_count":
              state_prep_gate_count,
          "controlled_dense_two_qubit_gate_count":
              gate_count_controlled,
          "controlled_dense_two_qubit_operation_name":
              CONTROLLED_DENSE_TWO_QUBIT_OPERATION_NAME,
          "layers":
              layers,
          "controlled_dense_two_qubit_gate_state_throughput_per_second":
              gate_count_controlled / median if median > 0 else None,
      })
    elif case == "controlled-swap-state":
      kernel, gate_count, state_prep_gate_count, controlled_swap_gate_count = (
          build_controlled_swap_state_kernel(cudaq, qubits, layers))
      action = lambda: cudaq.get_state(kernel)
      for _ in range(warmups):
        action()
      timings = timed_repeats(action, repeats)
      ...
      metrics.update({
          "gate_count": gate_count,
          "state_prep_gate_count": state_prep_gate_count,
          "controlled_swap_gate_count": controlled_swap_gate_count,
          "controlled_swap_gate_family": "built-in cswap",
          "layers": layers,
          "controlled_swap_gate_state_throughput_per_second":
              controlled_swap_gate_count / median if median > 0 else None,
      })
