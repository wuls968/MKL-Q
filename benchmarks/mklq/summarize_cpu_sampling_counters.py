...
SCHEMA_VERSION = "mklq-cpu-sampling-counter-summary-v1"
COUNTER_REPORT_SCHEMA_VERSION = "mklq-cpu-sampling-counter-probe-v1"
...
def boundary_summary(reports: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "performance_benchmark": any(
            ...
            for boundary in boundaries),
        "cross_machine_performance_proof": any(
...
def build_report_digest(path: Path, report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "cross_machine_performance_proof": bool(
            boundary.get("cross_machine_performance_proof")),
    }
...
def build_summary(paths: list[Path]) -> dict[str, Any]:
    ...
    status = (
        "passed" if loaded and totals["failed"] == 0 and totals["missing"] == 0
...
def render_markdown(summary: dict[str, Any]) -> str:
    ...
    for category in summary.get("categories", []):
        ...
        for test_name in category.get("tests", []):
            lines.append(
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Summarize bounded MKL-Q CPU sampling counter reports for public "
            "documentation."))
