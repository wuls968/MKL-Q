...
| Benchmark tooling | `test_mklq_benchmark_harness.py`, benchmark helper `py_compile`, summary JSON parse, `check_performance_evidence.py`, `check_sampling_profile_evidence.py`, `check_metal_evidence.py`, `run_cpu_sampling_counter_probe.py`, `summarize_cpu_sampling_counters.py`, `check_cpu_sampling_counter_docs.py`, `run_metal_runtime_counter_probe.py`, `summarize_metal_runtime_counters.py`, `check_metal_runtime_counter_docs.py` | Benchmark case/schema/report changes, tracked clean CPU evidence quality, tracked sampling profile evidence boundaries, tracked experimental Metal evidence boundary quality, bounded runtime counter evidence parsing, public counter-summary rendering, and counter-doc drift detection | Does not prove current runtime speed unless real benchmark rows run; sampling profile evidence is harness-level timing, not native backend internal counter evidence |
| Public release hygiene | `.github/workflows/mklq-public-hygiene.yml`, `run_preflight_audit.py`, `run_public_healthcheck.py`, `public-release-checklist.md` | Public metadata, docs, tracked artifact policy, expected Git remotes, local lock/artifact preflight, example file presence, local healthcheck composition, static clean CPU evidence guard, static Metal evidence boundary guard, bounded CPU/Metal runtime counter evidence parsing, and CPU/Metal counter docs drift detection | Does not build or run Apple Silicon backend tests unless `run_public_healthcheck.py --full --require-clean` is used locally |
