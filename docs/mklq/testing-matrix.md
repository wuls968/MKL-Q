...
| CPU sampling counter probe | `python3 benchmarks/mklq/run_cpu_sampling_counter_probe.py --build-dir build-python --output benchmarks/mklq/reports/local-cpu-sampling-counter-probe-YYYY-MM-DD.cpu-counter.json` | The complete expected build-tree `mklq_cpu_MKLQCpuTester.*NativePhases` CPU sampling phase counter-test set is present and each exact ctest asserts native sampling phase counters for full-register counts-only, partial-register counts-only, and sequential sampling routes | Timing performance, release sign-off, or cross-machine performance proof |
...
| `benchmarks/mklq/run_cpu_sampling_counter_probe.py` | CPU sampling counter probe | Requires the complete expected `mklq_cpu_MKLQCpuTester.*NativePhases` counter-test set, runs each selected ctest independently, and writes bounded `.cpu-counter.json` evidence without raw logs or release/performance claims |
