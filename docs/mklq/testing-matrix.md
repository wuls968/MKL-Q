...
| `benchmarks/mklq/bench_mklq_targets.py` | Target benchmark harness | Local timing rows for gate/state/sampling cases, QFT-like composite rows, CRZ distance distribution and per-distance sweep diagnostic rows, seeded Clifford stress rows, target notes, and static `mklq-metal` path-boundary labels |
