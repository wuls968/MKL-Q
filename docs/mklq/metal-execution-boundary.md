...
kernel, including a uniform-probability fast path that skips cumulative-weight
upload and binary search. Sequential stochastic sampling still records
host-side draw/count telemetry after resident probability work.
...
Metal device-generated draw/count accumulation paths and the selected
