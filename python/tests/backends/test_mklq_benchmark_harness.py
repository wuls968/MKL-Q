...
def test_mklq_clean_cpu_gate_plan_uses_fixed_environment(tmp_path):
    ...
    config = module.GateConfig(
        ...
        gate_cases=(
            ...
            "dense-two-qubit-state,three-qubit-state"),
        composite_cases=
    ...
    assert plan["paths"]["gate_raw"].endswith(
        ...
        "-dense-two-qubit-three-qubit-q20-2026-06-21.json")
...
def test_mklq_cpu_scaling_gate_plan_accepts_two_custom_dense_three_qubit_cases(
        tmp_path):
    module = _load_cpu_scaling_benchmark_gate_module()
    config = module.ScalingConfig(
        repo_root=tmp_path,
        ...
        cases=("two-qubit-state,custom-two-qubit-state,"
               "dense-two-qubit-state,three-qubit-state"),
        summary_id=
        "local-scaling-cpu-two-qubit-custom-two-qubit-dense-two-qubit-three-qubit-q18-q22-2026-07-08-dense-two-qubit-scaling",
        evidence_kind="clean_local_benchmark_evidence",
        ratio_group="clean_worktree_cross_target_ratio",
        performance_scope="local test only",
        summary_text=("Synthetic two/custom/dense/three-qubit scaling "
                      "benchmark gate."),
        runtime_note="synthetic runtime note",
        allow_dirty=False,
        skip_benchmark=False,
    ...
    assert plan["paths"]["raw"].endswith(
        "local-scaling-cpu-two-qubit-custom-two-qubit-dense-two-qubit-three-qubit-q18-q22-2026-07-08-dense-two-qubit-scaling.json"
    )
    assert plan["paths"]["summary"].endswith(
        "local-scaling-cpu-two-qubit-custom-two-qubit-dense-two-qubit-three-qubit-q18-q22-2026-07-08-dense-two-qubit-scaling.summary.json"
    )
    benchmark_command = plan["commands"]["raw"]
    assert benchmark_command[benchmark_command.index("--cases") + 1] == (
        "two-qubit-state,custom-two-qubit-state,dense-two-qubit-state,"
