...
def test_mklq_cpu_scaling_gate_plan_accepts_two_custom_controlled_dense_swap_three_qubit_cases(
    ...
    config = module.ScalingConfig(
        repo_root=tmp_path,
        pythonpath="/tmp/cudaq-runtime",
        stamp="2026-07-08-dense-two-qubit-scaling",
        qubits=[18, 20, 22],
        threads=10,
        repeats=3,
        warmups=1,
...
def test_mklq_benchmark_custom_two_qubit_case_records_gate_metrics(
    ...
    metrics = row["metrics"]
    assert metrics["state_prep_gate_count"] == 6
    assert metrics["custom_two_qubit_gate_count"] == 6
    assert metrics["gate_count"] == 12
    assert metrics["custom_two_qubit_operation_name"] == (
        module.CUSTOM_TWO_QUBIT_OPERATION_NAME)
    assert metrics["custom_two_qubit_gate_state_throughput_per_second"] == 12
    assert metrics["process_max_rss_bytes_cumulative"] == 4096
    assert fake_cudaq.registered_operations[
        module.CUSTOM_TWO_QUBIT_OPERATION_NAME] == (
            module.custom_two_qubit_matrix())
    operations = fake_cudaq.kernels[0].operations
    assert sum(1 for operation in operations
               if operation[0] == module.CUSTOM_TWO_QUBIT_OPERATION_NAME) == 6


def test_mklq_benchmark_dense_two_qubit_case_records_gate_metrics(monkeypatch):
    module = _load_benchmark_module()
    ...
    metrics = row["metrics"]
    assert metrics["state_prep_gate_count"] == 6
    assert metrics["dense_two_qubit_gate_count"] == 6
    assert metrics["gate_count"] == 12
    assert metrics["dense_two_qubit_operation_name"] == (
        module.DENSE_TWO_QUBIT_OPERATION_NAME)
    assert metrics["dense_two_qubit_gate_state_throughput_per_second"] == 12
    assert metrics["process_max_rss_bytes_cumulative"] == 4096
    assert fake_cudaq.registered_operations[
        module.DENSE_TWO_QUBIT_OPERATION_NAME] == (
            module.dense_two_qubit_matrix())
    operations = fake_cudaq.kernels[0].operations
    assert sum(1 for operation in operations
               if operation[0] == module.DENSE_TWO_QUBIT_OPERATION_NAME) == 6


def test_mklq_benchmark_controlled_dense_two_qubit_case_records_gate_metrics(
        monkeypatch):
    module = _load_benchmark_module()
    ...
    class FakeKernel:
        ...
        def control(self, target_kernel, control, *targets):
            self.operations.append(("control", target_kernel, control,
                                    *targets))
    ...
    class FakeCudaq:
        ...
        def make_kernel(self, *args):
            kernel = FakeKernel(*args)
            self.kernels.append(kernel)
            if args:
                return (kernel, *range(len(args)))
            return kernel
    ...
    assert row["status"] == "ok"
    metrics = row["metrics"]
    assert metrics["state_prep_gate_count"] == 7
    assert metrics["controlled_dense_two_qubit_gate_count"] == 6
    assert metrics["gate_count"] == 13
    assert metrics["controlled_dense_two_qubit_operation_name"] == (
        module.CONTROLLED_DENSE_TWO_QUBIT_OPERATION_NAME)
    assert (
        metrics["controlled_dense_two_qubit_gate_state_throughput_per_second"]
        == 12)
    assert metrics["process_max_rss_bytes_cumulative"] == 4096
    assert fake_cudaq.registered_operations[
        module.CONTROLLED_DENSE_TWO_QUBIT_OPERATION_NAME] == (
    ...
    subkernel, main_kernel = fake_cudaq.kernels
    assert subkernel.operations == [
        (module.CONTROLLED_DENSE_TWO_QUBIT_OPERATION_NAME, 0, 1)
    ]
    assert sum(1 for operation in main_kernel.operations
               if operation[0] == "control") == 6
...
def test_mklq_benchmark_dry_run_accepts_custom_two_qubit_case(tmp_path):
    repo_root = Path(__file__).resolve().parents[3]
    script = repo_root / "benchmarks" / "mklq" / "bench_mklq_targets.py"
    output = tmp_path / "dry-run-custom-two-qubit.json"
    env = os.environ.copy()
    env.pop("PYTHONPATH", None)
    ...
    report = json.loads(output.read_text(encoding="utf-8"))
    assert report["config"]["cases"] == ["custom-two-qubit-state"]
    rows = report["results"]
    assert len(rows) == 1
    assert rows[0]["status"] == "planned"
    assert rows[0]["case"] == "custom-two-qubit-state"
    assert rows[0]["estimated_state_bytes"] == 16 * (1 << 4)


def test_mklq_benchmark_dry_run_accepts_dense_two_qubit_case(tmp_path):
    repo_root = Path(__file__).resolve().parents[3]
    script = repo_root / "benchmarks" / "mklq" / "bench_mklq_targets.py"
    output = tmp_path / "dry-run-dense-two-qubit.json"
    env = os.environ.copy()
    env.pop("PYTHONPATH", None)
    ...
    report = json.loads(output.read_text(encoding="utf-8"))
    assert report["config"]["cases"] == ["dense-two-qubit-state"]
    rows = report["results"]
    assert len(rows) == 1
    assert rows[0]["status"] == "planned"
    assert rows[0]["case"] == "dense-two-qubit-state"
    assert rows[0]["estimated_state_bytes"] == 16 * (1 << 4)


def test_mklq_benchmark_dry_run_accepts_controlled_dense_two_qubit_case(
        tmp_path):
    repo_root = Path(__file__).resolve().parents[3]
    script = repo_root / "benchmarks" / "mklq" / "bench_mklq_targets.py"
    output = tmp_path / "dry-run-controlled-dense-two-qubit.json"
    env = os.environ.copy()
    env.pop("PYTHONPATH", None)
    ...
    assert len(rows) == 1
    assert rows[0]["status"] == "planned"
    assert rows[0]["case"] == "controlled-dense-two-qubit-state"
    assert rows[0]["estimated_state_bytes"] == 16 * (1 << 4)
...
def test_mklq_benchmark_summary_records_metal_count_accumulation_sampling():
    repo_root = Path(__file__).resolve().parents[3]
    summary_path = (
        repo_root / "benchmarks" / "mklq" / "reports" /
    ...
    assert summary["raw_results"][0]["path"] == (
        "benchmarks/mklq/results/"
        "local-metal-count-accumulation-sampling-q20-2026-07-04.json")
    assert summary["raw_results"][0]["sha256"] == (
        "751c59993fc4590b9729cc321ae1f328c0655b421b79ff596ddbaaf590b1bf8e")
    ...
    assert len(rows) == 8
    assert rows[("sample-full-register", 65536)]["metal_path_label"] == (
        "mklq_metal_full_register_sample_count_accumulation")
    assert "host-generated draws" in rows[
        ("sample-full-register", 65536)]["metal_path_scope"]
    ...
    assert summary["interpretation"]["partial_register_counts_accumulation"] == (
        "host-side draw/count accumulation")
    assert summary["interpretation"][
        "do_not_treat_as_clean_release_provenance"]


def test_mklq_benchmark_summary_records_metal_count_accumulation_q22():
    repo_root = Path(__file__).resolve().parents[3]
    summary_path = (
        repo_root / "benchmarks" / "mklq" / "reports" /
    ...
    assert summary["raw_results"][0]["sha256"] == (
        "0a270eda1a9725b5cddf27a292363fdb495770494c77b746b950eba17a4c0d33")
    assert summary["raw_results"][0]["status_rows"] == {"ok": 8}
    assert summary["config"]["targets"] == ["mklq-metal"]
