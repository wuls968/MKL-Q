...
struct FourQubitParams {
  std::uint64_t targetMask0;
  std::uint64_t targetMask1;
  std::uint64_t targetMask2;
  std::uint64_t targetMask3;
  std::uint32_t firstTarget;
  std::uint32_t secondTarget;
  std::uint32_t thirdTarget;
  std::uint32_t fourthTarget;
  std::uint32_t controlCount;
};
...
kernel void mklq_apply_four_qubit(
    device ComplexFloat *state [[buffer(0)]],
    constant ComplexFloat *matrix [[buffer(1)]],
    constant ulong *controlMasks [[buffer(2)]],
    constant FourQubitParams &params [[buffer(3)]],
    uint block [[thread_position_in_grid]]) {
  const ulong blockIndex = block;
  const ulong base = insertZeroBit(
      insertZeroBit(
  ...
  const ulong targetMasks[4] = {params.targetMask0, params.targetMask1,
                                params.targetMask2, params.targetMask3};
  ulong indices[16];
  ComplexFloat amplitudes[16];
  ComplexFloat updated[16];

  for (uint outcome = 0; outcome < 16; ++outcome) {
    ulong index = base;
    for (uint target = 0; target < 4; ++target)
      if ((outcome & (1u << target)) != 0)
        index |= targetMasks[target];
    indices[outcome] = index;
    amplitudes[outcome] = state[index];
  }

  for (uint row = 0; row < 16; ++row) {
    ComplexFloat value = ComplexFloat{0.0f, 0.0f};
    for (uint column = 0; column < 16; ++column)
      value = cadd(value, cmul(matrix[row * 16 + column],
                               amplitudes[column]));
    updated[row] = value;
  }

  for (uint outcome = 0; outcome < 16; ++outcome)
    state[indices[outcome]] = updated[outcome];
}
...
kernel void mklq_accumulate_sample_counts(
    ...
    constant SampleCountParams &params [[buffer(3)]],
...
kernel void mklq_accumulate_uniform_generated_sample_counts(
    device atomic_uint *counts [[buffer(0)]],
    constant GeneratedSampleCountParams &params [[buffer(1)]],
    uint drawIndex [[thread_position_in_grid]]) {
  if (drawIndex >= params.drawCount || params.outcomeCount == 0u)
    return;

  const float draw =
      mklq_uniform_unit(params.seed, drawIndex) * float(params.outcomeCount);
  const uint outcome = min(uint(draw), params.outcomeCount - 1u);
  atomic_fetch_add_explicit(&counts[outcome], 1u, memory_order_relaxed);
}
...
bool isUniformProbabilityDistribution(const double *probabilities,
                                      std::size_t probabilityCount,
                                      double totalWeight) {
  if (!std::isfinite(totalWeight) || !(totalWeight > 0.0))
    return false;
  const double expected = totalWeight / probabilityCount;
  if (!std::isfinite(expected) || !(expected > 0.0))
    return false;

  const double tolerance = std::max(1.0e-12, std::abs(expected) * 1.0e-4);
  for (std::size_t outcome = 0; outcome < probabilityCount; ++outcome) {
    const double probability = probabilities[outcome];
    if (!std::isfinite(probability) || probability < 0.0 ||
        std::abs(probability - expected) > tolerance)
      return false;
  }
...
struct MetalStateVectorExecutor::Impl {
  ...
  std::size_t residentProbabilityFillBufferAllocations = 0;
  ...
  Impl() {
    @autoreleasepool {
      ...
      function = [library newFunctionWithName:@"mklq_apply_four_qubit"];
      if (!function) {
        [library release];
        error = "failed to load mklq_apply_four_qubit Metal kernel.";
        return;
      }
      ...
      if (!fourQubitPipeline) {
        [library release];
        error = describeError(
            pipelineError,
            "failed to create Metal four-qubit compute pipeline.");
        return;
      }
      ...
      if (!function) {
        [library release];
        error = "failed to load mklq_accumulate_sample_counts Metal kernel.";
        return;
      }
      ...
      if (!sampleCountPipeline) {
        [library release];
        error = describeError(
            pipelineError,
            "failed to create Metal sample-count compute pipeline.");
        return;
      }
      ...
      function =
          [library newFunctionWithName:@"mklq_accumulate_uniform_generated_sample_counts"];
      if (!function) {
        [library release];
        error = "failed to load "
                "mklq_accumulate_uniform_generated_sample_counts Metal "
                "kernel.";
        return;
      }

      pipelineError = nil;
      uniformGeneratedSampleCountPipeline =
          [device newComputePipelineStateWithFunction:function
                                                error:&pipelineError];
      [function release];

      if (!uniformGeneratedSampleCountPipeline) {
        [library release];
        error = describeError(
            pipelineError,
            "failed to create Metal uniform generated sample-count compute "
            "pipeline.");
        return;
      }
  ...
  ~Impl() {
    ...
    [residentProbabilityBuffer release];
  ...
  bool available() const {
    return device && commandQueue && singleQubitPipeline && twoQubitPipeline &&
           threeQubitPipeline && fourQubitPipeline && probabilityPipeline &&
...
bool MetalStateVectorExecutor::uploadState(const std::complex<double> *state,
  ...
  @autoreleasepool {
    ...
    if (impl->residentStateSize != stateSize) {
      [impl->residentProbabilityBuffer release];
      impl->residentProbabilityBuffer = nil;
      impl->residentProbabilityBufferCapacity = 0;
    }
...
void MetalStateVectorExecutor::releaseResidentState() {
  if (!impl)
    return;
  (void)impl->flushResidentGateCommands();
  [impl->residentStateBuffer release];
...
bool MetalStateVectorExecutor::applyResidentFourQubitGate(
  ...
  if (!impl || !impl->available())
    return false;
  if (!impl->residentStateBuffer || impl->residentStateSize < 16 ||
      !isPowerOfTwo(impl->residentStateSize) || !matrix || !targetQubits) {
    impl->error = "invalid Metal resident four-qubit gate input.";
    return false;
  }
  for (std::size_t target = 0; target < 4; ++target) {
    if (!qubitWithinState(targetQubits[target], impl->residentStateSize)) {
      impl->error = "target qubit exceeds Metal state range.";
      return false;
    }
    for (std::size_t previous = 0; previous < target; ++previous) {
      if (targetQubits[target] == targetQubits[previous]) {
        impl->error = "duplicate Metal four-qubit gate target.";
        return false;
      }
    }
  }
  if (!fitsKernelThreadIndex(impl->residentStateSize >> 4)) {
    impl->error = "state size exceeds Metal four-qubit thread index range.";
    return false;
  }
  if (controlCount > 0 && !controlQubits) {
    impl->error = "missing Metal control qubit input.";
  ...
  std::array<MetalComplexFloat, 256> gpuMatrix;
  for (std::size_t index = 0; index < gpuMatrix.size(); ++index)
    gpuMatrix[index] =
        MetalComplexFloat{static_cast<float>(matrix[index].real()),
                          static_cast<float>(matrix[index].imag())};

  std::array<std::size_t, 4> sortedTargets{
      targetQubits[0], targetQubits[1], targetQubits[2], targetQubits[3]};
  std::sort(sortedTargets.begin(), sortedTargets.end());
  FourQubitParams params{
      std::uint64_t{1} << targetQubits[0],
      std::uint64_t{1} << targetQubits[1],
      std::uint64_t{1} << targetQubits[2],
      std::uint64_t{1} << targetQubits[3],
      static_cast<std::uint32_t>(sortedTargets[0]),
      static_cast<std::uint32_t>(sortedTargets[1]),
      static_cast<std::uint32_t>(sortedTargets[2]),
      static_cast<std::uint32_t>(sortedTargets[3]),
      static_cast<std::uint32_t>(controlCount)};
  ...
  @autoreleasepool {
    ...
    if (!matrixBuffer || !controlsBuffer || !paramsBuffer) {
      ...
      [controlsBuffer release];
      [paramsBuffer release];
      impl->error = "failed to allocate Metal resident four-qubit buffers.";
      return false;
    }
    ...
    [encoder setComputePipelineState:impl->fourQubitPipeline];
    [encoder setBuffer:impl->residentStateBuffer offset:0 atIndex:0];
    [encoder setBuffer:matrixBuffer offset:0 atIndex:1];
    [encoder setBuffer:controlsBuffer offset:0 atIndex:2];
    [encoder setBuffer:paramsBuffer offset:0 atIndex:3];

    const auto blockCount = impl->residentStateSize >> 4;
    const auto pipelineWidth =
        [impl->fourQubitPipeline maxTotalThreadsPerThreadgroup];
    const auto threadsPerThreadgroup =
        std::max<NSUInteger>(1, std::min<NSUInteger>(pipelineWidth, blockCount));
    [encoder dispatchThreads:MTLSizeMake(blockCount, 1, 1)
       threadsPerThreadgroup:MTLSizeMake(threadsPerThreadgroup, 1, 1)];
...
bool MetalStateVectorExecutor::fillResidentFullRegisterProbabilities(
  ...
  @autoreleasepool {
    if (!impl->residentProbabilityBuffer ||
        impl->residentProbabilityBufferCapacity < impl->residentStateSize) {
      const auto bufferPreparationStart = std::chrono::steady_clock::now();
      id<MTLBuffer> probabilitiesBuffer = [impl->device
          newBufferWithLength:impl->residentStateSize * sizeof(float)
                       options:MTLResourceStorageModeShared];
      ...
      [impl->residentProbabilityBuffer release];
      impl->residentProbabilityBuffer = probabilitiesBuffer;
      impl->residentProbabilityBufferCapacity = impl->residentStateSize;
      ++impl->residentProbabilityFillBufferAllocations;
    ...
    id<MTLBuffer> probabilitiesBuffer = impl->residentProbabilityBuffer;
...
bool MetalStateVectorExecutor::accumulateSampleCounts(
    const double *probabilities, std::size_t probabilityCount,
    const double *draws, std::size_t drawCount, std::uint32_t *counts,
    std::size_t countBufferSize) {
  if (!impl || !impl->available())
    return false;
  if (!probabilities || !draws || !counts || probabilityCount == 0 ||
      drawCount == 0 || countBufferSize != probabilityCount) {
    impl->error = "invalid Metal sample-count accumulation input.";
    return false;
  }
  if (probabilityCount > std::numeric_limits<std::uint32_t>::max() ||
      drawCount > std::numeric_limits<std::uint32_t>::max()) {
    impl->error = "Metal sample-count input exceeds index range.";
    return false;
  }
  ...
  if (!(cumulative > 0.0)) {
    impl->error = "Metal sample-count accumulation requires positive weight.";
    return false;
  }

  std::vector<float> gpuDraws(drawCount, 0.0f);
  for (std::size_t draw = 0; draw < drawCount; ++draw)
    gpuDraws[draw] = static_cast<float>(draws[draw]);

  std::vector<std::uint32_t> gpuCounts(probabilityCount, 0);
  const SampleCountParams params{static_cast<std::uint32_t>(probabilityCount),
  ...
  @autoreleasepool {
    id<MTLBuffer> cumulativeBuffer =
        [impl->device newBufferWithBytes:cumulativeWeights.data()
                                  length:cumulativeWeights.size() *
                                         sizeof(float)
                                 options:MTLResourceStorageModeShared];
    id<MTLBuffer> drawsBuffer =
        [impl->device newBufferWithBytes:gpuDraws.data()
                                  length:gpuDraws.size() * sizeof(float)
                                 options:MTLResourceStorageModeShared];
    ...
    id<MTLBuffer> paramsBuffer =
        [impl->device newBufferWithBytes:&params
                                  length:sizeof(SampleCountParams)
                                 options:MTLResourceStorageModeShared];

    if (!cumulativeBuffer || !drawsBuffer || !countsBuffer || !paramsBuffer) {
      [paramsBuffer release];
      [countsBuffer release];
      [drawsBuffer release];
      [cumulativeBuffer release];
      impl->error = "failed to allocate Metal sample-count buffers.";
      return false;
    }
    ...
    if (!commandBuffer || !encoder) {
      ...
      [drawsBuffer release];
      [cumulativeBuffer release];
      impl->error = "failed to create Metal sample-count command encoder.";
      return false;
    }

    [encoder setComputePipelineState:impl->sampleCountPipeline];
    [encoder setBuffer:cumulativeBuffer offset:0 atIndex:0];
    [encoder setBuffer:drawsBuffer offset:0 atIndex:1];
    [encoder setBuffer:countsBuffer offset:0 atIndex:2];
    [encoder setBuffer:paramsBuffer offset:0 atIndex:3];

    const auto pipelineWidth =
        [impl->sampleCountPipeline maxTotalThreadsPerThreadgroup];
    const auto threadsPerThreadgroup =
    ...
    if (status != MTLCommandBufferStatusCompleted) {
      impl->error = describeError(
          [commandBuffer error], "Metal sample-count command buffer failed.");
      [paramsBuffer release];
      [countsBuffer release];
      [drawsBuffer release];
      [cumulativeBuffer release];
...
bool MetalStateVectorExecutor::accumulateGeneratedSampleCounts(
  ...
  const double totalWeight =
      std::accumulate(probabilities, probabilities + probabilityCount, 0.0);
  if (!(totalWeight > 0.0)) {
    impl->error =
  ...
  if (isUniformProbabilityDistribution(probabilities, probabilityCount,
                                       totalWeight)) {
    std::vector<std::uint32_t> gpuCounts(probabilityCount, 0);
    ...
    @autoreleasepool {
      ...
      if (!countsBuffer || !paramsBuffer) {
        [paramsBuffer release];
        [countsBuffer release];
        impl->error =
            "failed to allocate Metal uniform generated sample-count buffers.";
        return false;
      }
      ...
      if (!commandBuffer || !encoder) {
        ...
        [countsBuffer release];
        impl->error =
            "failed to create Metal uniform generated sample-count command "
            "encoder.";
        return false;
      }

      [encoder
          setComputePipelineState:impl->uniformGeneratedSampleCountPipeline];
      [encoder setBuffer:countsBuffer offset:0 atIndex:0];
      [encoder setBuffer:paramsBuffer offset:0 atIndex:1];

      const auto pipelineWidth =
          [impl->uniformGeneratedSampleCountPipeline
              maxTotalThreadsPerThreadgroup];
      const auto threadsPerThreadgroup = std::max<NSUInteger>(
          1, std::min<NSUInteger>(pipelineWidth, drawCount));
      [encoder dispatchThreads:MTLSizeMake(drawCount, 1, 1)
         threadsPerThreadgroup:MTLSizeMake(threadsPerThreadgroup, 1, 1)];
      [encoder endEncoding];
      [commandBuffer commit];
      ...
      if (status != MTLCommandBufferStatusCompleted) {
        impl->error = describeError(
            [commandBuffer error],
            "Metal uniform generated sample-count command buffer failed.");
        [paramsBuffer release];
        [countsBuffer release];
        return false;
      }
    ...
    ++impl->uniformGeneratedSampleCountAccumulations;
    return true;
  }
...
std::size_t MetalStateVectorExecutor::fourQubitGateApplications() const {
  return impl ? impl->fourQubitApplications : 0;
}
...
MetalStateVectorExecutor::residentProbabilityFillBufferAllocations() const {
  return impl ? impl->residentProbabilityFillBufferAllocations : 0;
}
...
MetalStateVectorExecutor::uniformGeneratedSampleCountAccumulations() const {
  return impl ? impl->uniformGeneratedSampleCountAccumulations : 0;
}
