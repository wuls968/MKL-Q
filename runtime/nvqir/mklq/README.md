...
- `mklq-cpu` is a correctness-first fp64 state-vector baseline with initial
  ...
  Single-control X/CNOT and H/Y/Rx/Ry/Rz use direct target/control pair
  enumeration instead of a per-pair control predicate, including
  single-control Rz phase updates. Built-in X/CNOT use a dedicated in-place
  ...
  for both counts-only and explicit sequential results. Non-deterministic
  counts-only sampling (`includeSequentialData=false`) aggregates drawn outcome
  counts before converting bit strings, using a bounded dense counter buffer
  for small outcome spaces and a sparse map for larger ones.
...
- `mklq-metal` remains experimental. It no longer routes through the upstream
  ...
  generated-count fast path. Sequential stochastic sampling remains host-side;
  build-tree counter tests cover the current split between selected counts-only
  Metal draw/count accumulation and host-side sequential draw telemetry.
